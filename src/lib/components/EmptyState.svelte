<script lang="ts">
  export let mode: 'learning' | 'reviews';
  export let onModeSwitch: (mode: 'learning' | 'reviews') => void;
...
<div class="flex flex-col items-center justify-center py-12 text-center">
  {#if !hasLearning && !hasReviews}
    <!-- Completed all words -->
    <div class="mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-r from-green-400 to-blue-500">
    ...
    <div class="mt-6 rounded-lg bg-yellow-50 p-4">
      <p class="text-sm text-yellow-800">
        Keep reviewing your learned words to maintain your knowledge, or check back later for new content!
      </p>
    </div>
  {:else if mode === 'learning' && !hasLearning}
    ...
    {:else}
      <div class="rounded-lg bg-green-50 p-4">
        <p class="text-sm text-green-800">
          No reviews are due yet. Take a break and come back later, or check your progress in settings.
        </p>
      </div>
    {/if}
  {:else if mode === 'reviews' && !hasReviews}
    ...
    <h3 class="text-xl font-bold text-gray-900 mb-2">No reviews due!</h3>
    <p class="text-gray-600 mb-4 max-w-md">
      All your words are still fresh in memory. Come back later for spaced repetition reviews.
    </p>
    {#if hasLearning}
      <button
        on:click={() => onModeSwitch('learning')}
        class="inline-flex items-center rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors"
      >
        <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
  ...
  <div class="mt-8 max-w-md text-left">
    <h4 class="text-sm font-medium text-gray-900 mb-3">💡 Study Tips</h4>
