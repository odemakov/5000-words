...
export const queueStats = derived<typeof learningState, QueueStats>(
  learningState,
  ($state) => {
    const now = Date.now();
    const dueWords = [...$state.recap7, ...$state.recap14, ...$state.recap30]
...
export class LearningController {
  ...
  static async initializeLearning(detectedLevel: 'A1' | 'A2' | 'B1' | 'B2', testResults: Array<{ wordId: number; known: boolean }>) {
    ...
    learningState.update(state => {
      ...
      if (known) {
        ...
        const recapItem: RecapWord = {
          wordIndex: card.wordIndex,
          addedAt: card.attempts ? state.forwardQueue.find(w => w.wordIndex === card.wordIndex)?.addedAt || now : now,
          attempts: (card.attempts || 0) + 1,
  ...
  static updateCurrentDirection() {
    learningState.update(state => {
      ...
      if (state.currentMode === 'learning') {
        if (state.currentDirection === 'forward' && state.forwardQueue.length === 0 && state.backwardQueue.length > 0) {
          return { ...state, currentDirection: 'backward' };
        } else if (state.currentDirection === 'backward' && state.backwardQueue.length === 0 && state.forwardQueue.length > 0) {
  ...
  static loadState(): LearningState | null {
    return Storage.getLearningState();
  }
